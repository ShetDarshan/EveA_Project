...
export const reguser = (userInfo,history) => dispatch => {
    axios
      .post('http://localhost:5001/evea-prj/us-central1/api/api/v1/register', userInfo)
      .then(res => {
...
export const luser = (userData) => (dispatch) => {
    axios
        .post('http://localhost:5001/evea-prj/us-central1/api/api/v1/login',userData)
        .then(res => {
            const { token } = res.data;
...
export const forgotpwd = (pwd,history) => (dispatch) =>{
    axios
        .post('http://localhost:5001/evea-prj/us-central1/api/api/v1/forgotpwd',pwd)   
        .then(
            history.push('/login')
        )    
        .catch(err => 
